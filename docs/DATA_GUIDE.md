...
- **`amount` 为真实成交额**（源 `成交额`，单位元），非美股库的 `close×volume` 退化估算。
- **`volume` 单位为手（1 手 = 100 股）**，与美股库的股数口径不同（源 `成交量` 即为手数，
  库内原样保留）。据此 `amount / volume` = 每股价格 × 100。
- **指数无成交量**，`volume` 记 0；`amount` 仍为真实成交额。`trade_count` 源无此列，记 0。
- **分钟表比美股库多 `vwap` / `amount` 两列**：`amount` 忠实保存源逐分钟成交额（删除源 CSV 后
  逐分钟成交额即由此保留）；`vwap`=成交额/成交量（元/手，成交量为 0 含指数时记 0）。这样
  AlphaForge 面板 `amount = vwap × volume` **精确**还原真实成交额（非美股路径的 `close×volume`
  近似）。注意 `vwap` 因 volume 以手计而等于每股价格 × 100；如需每股 VWAP，用 `amount /
  (volume × 100)`。
