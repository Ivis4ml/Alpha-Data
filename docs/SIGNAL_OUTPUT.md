...
**产物位置**：`docs/signal_summary.json`（受 Git 跟踪）。结构为
`{"meta": {...}, "records": [8 条]}`；**全部量化字段为 JSON number、
不适用为 null**（供下游程序直接处理），单位在字段名后缀（`_bp`）或
`meta.units` 中声明；正文表格由报告构建时读同一 JSON 格式化渲染。

示例记录（节选，纯数字）：

```json
{"交易品种": "SC", "信号": "JUMP_SC_day", "信号类型": "离散",
 "信号月次数": 24.02,
 "离散信号（取1）后10分钟平均收益": 10.634,
 "取值valuecount": {"-1": 134, "1": 143}, "取值非零频率": 0.00409,
 "品种全区间收对收累计收益": 0.088,
 "品种无条件10分钟均值收益_bp": -0.1264}
```
...
| 取值valuecount / 取值非零频率 | 离散信号逐值计数（对象 {取值: 次数}）与触发频率（小数） |
| 直方图 | `docs/figures/f_signal_summary_hist.png`（面板号在字段内） |
| 品种全区间收对收累计收益 / 品种无条件{1,3,10}分钟均值收益_bp | 四个独立数值字段：累计为小数（0.088=+8.8%，剔换月日），无条件均值为 bp，作条件均值的对照基准 |
