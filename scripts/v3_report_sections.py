...
{fig_tag("v3/f_v3_oos.png",
         ...
         "多数品种为负；唯一稳定为正的是豆粕 M。同维度对比下 v3 精简（绿）"
         "在 6/8 品种上优于基线（灰）；全测量层（蓝）过参数化更差。")}
<ul>
<li><b>绝对水平：两套信号对次日方向的 OOS 增量预测力整体为负。</b>唯一例外
是豆粕 M：v3 信号 +4.9%（CW t = 1.86，块自助 p ≈ 0.001，前后半段 +4.2% /
+6.2% 同号），对全部六项修复稳健。但跨 8 品种 BH-FDR q = 0.12，且其控制
用 SOYB（大豆 ETF）代理豆粕、缺 USDCNH——定级为<b>候选发现，非结论</b>。</li>
