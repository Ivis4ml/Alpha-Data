...
<h2 id="s15">15　测量层：从成交流到潜在信念结构（point-in-time 协议）</h2>
...
"信息增量"单位。</div>
<p><b>point-in-time 协议（v3.1 修订）</b>：初版用每个市场完整生命周期估计
(q, r_0)，再回头生成早期滤波状态——即使从不接触期货收益，也让 5 月的信号
构造"知道"了 6 月的 Polymarket 数据（时间前视，区别于目标泄漏）。修订后
...
<h3>15.3 Layer 3：hazard、分布特征与 LOFO 平台公共因子</h3>
...
regime-fall 也在同一收益窗。市场级回归把一个收益观测复制了 25 次，
...
ex-ante 与 ex-post 两套门控一致拦截，Layer 4 整体降级 sign-only。</b>初版
...
{fig_tag("v3/f_v3_episode.png",
         ...
         "|s|>0.2 的 episode 仅 6 个——即便门控放行，系数也几乎由这 6 天决定。")}
<p><b>门控分 ex-ante 与 ex-post 两套</b>（评审第 4 点）：决定 OOS 管道模型
结构的门控只用训练截止（05-15）前的 episode；全样本版本仅回答"以当前数据量
哪些参数可研究"，不得反向影响模型选择。本轮两套判定一致（都拦截），随爬虫
日常增量与新事件周期积累，ex-post 表按月复核——影响系数何时可识别由数据
说话，门控继续挂在管道上。</p>
<p><b>已知的剩余粗糙度</b>：episode 键按"北京 09:00 前算当日、其后算次一
交易日"归并到<b>日</b>粒度；对有夜盘品种，严格的下一可交易时点常是当晚
21:00 而非次日 09:00，episode 收益窗因此混合了夜盘与日盘吸收。在门控全部
拦截的现状下该细化不改变任何决策（它只会进一步收紧样本），Layer 5 的时段
拆分设计（§17.2）已沿真实交易序列分解了吸收位置；待 episode 数积累到接近
门控阈值时，此对齐须先于任何 β 估计完成。</p>
...
窗口收益 + SPY + UUP + 自身滞后 + 平台全局因子。展开窗口一步 OOS，最少 40
...
{fig_tag("v3/f_v3_oos.png",
         "OOS 增量 R²（相对控制集 M0，%）。Clark-West 为单侧检验：▲ = t ≥ 1.645"
         "（扩展模型显著改善），▼ = t ≤ −1.645（显著恶化），二者不共用记号。"
         "多数品种为负；唯一显示正向 OOS 增量的候选品种是豆粕 M。同维度对比下 "
         "v3 精简（绿）在 6/8 品种上优于基线（灰）；全测量层（蓝）过参数化更差。")}
<ul>
<li><b>绝对水平：两套信号对次日方向的 OOS 增量预测力整体为负。</b>唯一显示
正向 OOS 增量的候选品种是豆粕 M：v3 信号 +4.9%（CW 单侧 t = 1.86，块自助
p ≈ 0.001，前后半段 +4.2% / +6.2% 同号），对全部六项修复稳健。但跨 8 品种
BH-FDR q = 0.12，且其控制用 SOYB（大豆 ETF）代理豆粕、缺 USDCNH——定级为
<b>候选发现，非结论</b>。注意 Clark-West 的解读是单侧的：AG 基线的
t = −2.18 属"扩展模型显著恶化"，与 M 的正 t 不是同一种"显著"。</li>
<li><b>相对改善：v3 精简在 6/8 品种上优于基线</b>（修复时间前视与 LOFO 后
仍成立），但跨品种合并的标准化损失差块自助单侧 p = 0.147——<b>方向性
改善，统计上不足为证</b>。6/8 的符号计数本身不构成显著性声称。</li>
<li>CF 的 CW t = +1.99 与负 ΔR² 并存：扣除参数噪声后信号存在、但估计成本
超过收益的典型 Clark-West 形态，不作方向声称。</li>
...
MDE 超限（最近差 38%）。功效门控从"装饰"变成了实际拦截了一次已发表错误
的机制。</div>
<div class="finding"><span class="no">三</span><b>豆粕 M 的正 OOS 增量是唯一
...
功效核算输入与防前视协议本身，不在 OOS 增量的统计显著。</div>
...
经济上限即降级</td></tr>
<tr><td>sign-only</td><td>功效不足时的回退：只用方向先验 × 标准化信号</td></tr>
...
<tr><td><code>scripts/v3_build_tape.py</code></td><td>统一 tape 构建 + 登记表</td></tr>
<tr><td><code>scripts/v3_measurement.py</code></td><td>Layer 1-3 测量层
（PIT + LOFO，约 2 分钟）</td></tr>
<tr><td><code>scripts/v3_baseline_extended.py</code></td><td>v1.1 基线信号在
扩展样本上的重建（约 22 分钟）</td></tr>
<tr><td><code>scripts/v3_layer5.py</code></td><td>episode 双门控 + Layer 4/5
评估 + 多重检验</td></tr>
<tr><td><code>data/cn_futures/analysis/v3/</code></td><td>信号面板、两套功效表、
episode 表、OOS 与稳健性、吸收复核、H4 等 20 个产物表</td></tr>
