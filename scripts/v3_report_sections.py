...
def oos_table_html() -> str:
    ...
    head = ("<tr><th>品种</th><th>OOS 天数</th><th>v1.1 基线</th>"
            ...
            "<th>CW t（精简）</th><th>BH-FDR q</th><th>块自助（block bootstrap）p</th>"
            "<th>前 / 后半段</th></tr>")
    rows = []
    for r in tab.itertuples(index=False):
...
<div class="pt">v3 识别（identification）与统计功效框架：分阶段准入、扩展样本与一次真实的证伪</div>
...
指出事件伪重复、测量参数时间前视（look-ahead bias）、事前与事后功效门槛混用，以及公共因子含自身
...
<h3>15.1 第一层：logit 状态空间滤波（state-space model）</h3>
<p>第一部分的信号是"15 分钟桶两向加权中位数的 logit 之差"——直接、稳健，
但每个市场的创新（innovation）没有统一的噪声尺度。v3 把它升级为状态空间模型：
观测是桶价的 logit，真实信念是不可观测的状态，两者之差是微观结构噪声
（market microstructure noise），其方差由桶内可观测量决定：</p>
{T[0]}
...
<h3>15.3 第三层：结算风险率（hazard rate）、分布特征与留一主题平台公共因子</h3>
...
{T[5]}
<div class="texnote">系数 b 只用严格早于当期的观测（展开窗口，expanding window，无前视）。
