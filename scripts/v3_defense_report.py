...
——真实但小，属分钟级事件信号的正常水平。<b>第五轮外部审计降格
（重要）</b>：双腿分解显示该稳定性主要由"−z(过去 120 分钟期货收益)"的
期货自身反转腿承载（反转腿单独 t 8-11），PM 腿单独在按交易日聚类的
推断下于 SC / AU / CU / M 均不显著、AG 显著为负；C8 应读作"PM 变化 +
期货短期反转"的复合信号，Polymarket 的分钟级增量未证实，详见主报告
§12.14。</div>
...
+9.8 bp 经事件桶折叠 + 交易日聚类后为 +6.9 bp（HAC t = 1.68，仅
10% 水平）；时段分类未用真实期货交易日历（约 58% 精确映射、28%
延迟超 60 分钟）。本节全部结论按<b>探索性证据</b>解读，方向性叙述
保留，正确推断（事件 / 日双重聚类、真实日历）登记为待办，详见主报告
§12.14。</p>
