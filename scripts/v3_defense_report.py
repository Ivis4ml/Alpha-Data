...
检验 -> 诚实结论），全部解释性内容下沉到五个附录（Polymarket 完全指南 /
FAQ / 字段词典 / 与导师方法论文档逐条对照 / 复现索引）。
...
def qa(q: str, a: str) -> str:
    return f'<div class="qa"><b>{q}</b>　{a}</div>'
...
<b>附录是主线的 supplement</b>：完全不了解 Polymarket 的读者请先读
<a href="#appA">附录 A（写给期货研究员的 Polymarket 完全指南）</a>；
...
一方每份兑付 1 USDC（美元稳定币，1 USDC ≈ 1 美元；脱锚风险见附录 B）；
<b>代币价格 ∈ (0,1) 即市场隐含概率</b>（机制推导见
附录 A.3）。挂单撮合在运营方链下引擎完成，<b>成交清算上链</b>——我们爬取
...
Yes 价 + No 价恒等于 1（由铸造 / 销毁套利钉住，真实数据见附录 A.4），故
...
没跟上」的<b>连续缺口幅度</b>里（C8，§4.3）——两节互为印证。
...
def appendix_a() -> str:
    ...
    return f"""
<h2 id="appA">附录 A　写给期货研究员的 Polymarket 完全指南</h2>

<h3>A.1 它是什么：与股票 / 期货的对照表</h3>
...
盘口）。吃单量超过最优价档位深度时，撮合引擎自动路由到下一档更差的价格
——这就是「买盘推高价格」的机械过程。§1.1 的真实订单是教科书案例：卖
7,190 份，0.974 一档只有 2,223 份买盘，剩余 4,967 份被迫吃到 0.973。与
期货的差异只有两点：挂单是链下签名消息（挂单免 gas、可随时撤），成交清算
上链。</p>

<h3>A.3 为什么价格 = 概率</h3>
<p>一份 Yes 代币的结算支付：事件发生兑 1 USDC、不发生兑 0（合约硬编码）。
若你认为事件概率是 p，其期望价值恰为 p × 1 + (1−p) × 0 = p。价格高于 p
买方期望亏，低于 p 卖方期望亏，双向下注把价格推向 p——所以<b>价格直接可
读为概率，无需定价模型换算</b>（对比：期权价格要反解隐含波动率）。No 代币
价 0.974 读作「市场付 97.4 美分赌不发生」，即 P(发生) ≈ 2.6%。已知偏差
（favorite-longshot 等）见附录 B 第 3 问。</p>

<h3>A.4 Yes + No = 1：套利如何维持、联动有多快</h3>
...
<h3>A.6 谁在交易：匿名钱包与操纵问题</h3>
<p>参与者是匿名链上地址：做市商程序（双边报价赚价差）、方向性投机者、以及
<b>事件知情者</b>——预测市场不禁止「内幕交易」，知情资金入场恰是价格信息
含量的来源。操纵可能吗？短时可以（小市场上用钱推价格），但成本高昂：推离
真值的头寸就是给套利者与知情者的补贴，结算按真实结果清算。我们的缓解：
$10 万准入门槛、流动性加权、分钟 vwap 聚合，以及主报告的稳健性检验族
（置换检验 / 安慰剂 / 控制变量）。</p>
"""
...
def appendix_b() -> str:
    faqs = [
        ("这不就是赌博吗？数据能用于严肃研究？",
         "机制上是「用钱表达概率判断」的市场，与期货价格发现同构（CME 亦已"
         "上市选举期货）。预测市场校准性有大量学术文献。本研究只把它当"
         "<b>信息源</b>（事件概率的高频测量仪），不涉及参与交易。"),
        ("数据会被平台改吗？",
         "不能。成交清算在 Polygon 链上，历史区块不可篡改；我们直接从链上"
         "事件抓取，不经过平台 API。"),
        ("价格 = 概率有多可信？有没有系统性偏差？",
         "有已知偏差：favorite-longshot bias（小概率被高估、大概率被低估）。"
         "本研究用概率的<b>变化</b>（logit 创新）而非水平，水平偏差在差分中"
         "大部分被消掉。"),
        ("大户能操纵概率吗？",
         "短时能、持续贵（附录 A.6）。且我们检验的是与期货的关联——操纵者"
         "若不能同时操纵上海原油期货，关联检验不受单侧操纵支配。"),
        ("有内幕交易吗？",
         "有，且不被禁止。这对研究是特性而非缺陷：知情流入场使价格先于新闻"
         "反映信息。"),
        ("怎么做空一个事件？", "买 No 代币，或卖出持有的 Yes。无融券概念。"),
        ("有杠杆吗？会爆仓吗？",
         "无杠杆：全额 USDC 抵押，最大损失 = 本金，无保证金追缴与强平。"),
        ("有涨跌停吗？",
         "无。价格天然限于 (0,1)；事件发生时一夜从 0.03 跳到 0.9 完全可能。"),
        ("交易时间？",
         "7×24×365。这正是它对中国闭市时段有信息价值的原因。"),
        ("手续费多少？",
         "多数市场基础费率为 0，部分市场有 taker 费。§1.1 示例交易的 fee "
         "字段 = 0（中继腿上记总费）。"),
        ("流动性多大？",
         "头部事件市场单市场累计成交数千万至上亿美元（中东主题合计约 $17 "
         "亿）；长尾很薄。用 $10 万准入 + 流动性加权处理。"),
        ("点差多大？",
         "无历史盘口（见下问），用同分钟买卖两向成交中位差代理：头部市场约 "
         "0.001-0.005（0.1-0.5 个百分点概率）。"),
        ("为什么没有挂单 / 盘口数据？",
         "挂单是链下签名消息，只有成交才上链。<b>任何人都无法从链上重建历史"
         "订单簿</b>；实时盘口 API 无历史回溯。这是本数据的结构性边界，也"
         "决定了点差只能用成交两向中位差代理。"),
        ("一个事件怎么有几十个市场？",
         "平台按「截止日阶梯」（by 2/28、by 3/31 …）与「阈值阶梯」（油价 "
         "hit $100 / 110 …）批量出题。研究上必须做族处理，否则一个现实事件"
         "被数成几十个样本（主报告曾因此撤回一个结论，见其 §16）。"),
        ("Yes 和 No 的价格会对不上吗？",
         "分钟级会（两本独立簿），十几分钟内被套利拉回——附录 A.4 有完整的"
         "真实数据时间线。"),
        ("为什么爬链而不用官方 API？",
         "完整性（全历史逐笔）、可验证性（任何人可复算）、无速率限制；官方 "
         "API 不承诺历史逐笔。代价是自己处理中继腿等清洗（§1.6）。"),
        ("USDC 是什么？会脱锚吗？",
         "美元稳定币（1 USDC ≈ $1）。极端脱锚（2023-03 硅谷银行事件短暂 "
         "$0.88）会让「概率」定价短暂失真——样本期内无此事件，列为已知外部"
         "风险。"),
        ("数据里有刷量吗？",
         "中继腿（记账重复）已剔除；铸造 / 销毁不算成交；同主体自成交未专门"
         "识别（§1.6 P12）——量类信号谨慎解读，方向类信号天然免疫。"),
        ("平台跑路 / 停服怎么办？",
         "架构级变更已经历一次（2026-04-28 v1 → v2 交易所迁移），爬虫双合约"
         "并抓、迁移期同质性实测通过——链上数据的好处是平台行为可被独立"
         "验证。"),
        ("市场结算错了怎么办？",
         "UMA 争议机制可挑战；极少数市场重报，我们取首次结算事件并如实记录"
         "（§1.6 P6）。"),
        ("这些市场和中国期货有什么关系？",
         "映射靠事前注册规则：中东冲突 → 原油 / 黄金（供给风险 / 避险）、"
         "美联储 → 贵金属 / 铜（利率敏感）、中美贸易 → 豆粕 / 铜（进口"
         "结构）。方向先验 σ 在准入时注册、不事后调整（主报告 §3）。"),
        ("为什么不直接用新闻做事件信号？",
         "新闻无概率读数、无金钱成本、时间戳口径混乱。预测市场把「事件信息」"
         "变成带出价的连续时间序列——这是本研究的方法论前提。"),
        ("分钟级 RankIC 才 0.03，有意义吗？",
         "意义在结构（哪类信号、什么状态有效）而非单格大小，读法见 §4.3 与 "
         "§6；能否变成可交易 alpha 由主报告的成本 / 功效框架回答。"),
        ("和主报告结论矛盾吗（那边说日频预测力不行）？",
         "不矛盾：分钟级渗入与隔日不可预测恰好互补，两命题的关系见 §6。"),
    ]
    items = "".join(qa(q_, a_) for q_, a_ in faqs)
    return f"""
<h2 id="appB">附录 B　FAQ：评审可能问的 24 个问题</h2>
{items}
"""
...
cn_futures_polymarket_report.html（v2.1）互补 · 附录 A.4 套利时间线的复现：
