...
def build() -> str:
    ...
    supp_interaction = v3_supp_sections.sec_interaction()
    supp_eventfix = v3_supp_sections.sec_event_correction()
    supp_rvoos = v3_supp_sections.sec_rv_oos()
    supp_btattrs = v3_supp_sections.sec_backtest_attrs()
    supp_combo = v3_supp_sections.sec_combo()
...
报告 q 值。<b>局部投影</b>（local projections，Jordà 2005）：对每个视界单独回归，见第 7 节。</p>
...
<h2 id="s10">10　事件研究（event study）</h2>
...
符号化后平均；置信带 = 500 次事件重抽自助法（bootstrap）。</p>
<p><b>勘误（答辩审计发现）</b>：
...
<h3>12.2 安慰剂检验（placebo test）与循环置换（permutation test）
（结果是否品种特异、是否统计巧合）</h3>
...
不构成样本外（out-of-sample, OOS）证据；S3 本质是在做单一事件期的 SC 升水均值回归。</p>
...
（mediation analysis，Baron &amp; Kenny 1986 分解）回答——总效应 c（PM→CN）、
路径 a（PM→美股，窗口内）、路径 b（美股→CN，
...
<h3>12.8 美股作为代理：充分性框架（潜因子模型，latent factor model，
与四组实验）</h3>
...
遗漏变量恒等式（omitted variable bias formula，线性代数上精确成立，
表中"恒等式闭合"列为数值验证，
应为机器零）：</p>
{_TEX[13]}
{suff_html}
...
（+2.47）高于单一 USO 后（+1.88）属共线抑制效应（collinearity-induced
suppression effect）且 n=37 对 13 个回归元自由度
...
<div class="finding"><span class="no">四</span>事件层面的 v1.1 结论经勘误修正后
<b>撤回</b>：「新市场创建最强（120 分钟 +41bp）」由视界标注错误、E3 符号
未实现（硬编码 +1 吸收冲突期漂移）与无基线三个缺陷制造，修正版 E3 的 CAR
区间含零且点估计翻负（§10b）；保留的只有 E1 价格跳 5 分钟视界的微弱正超额
——"几分钟内吸收完毕、无分钟级可利用领先"的定性结论以修正版为准。</div>
...
<div class="finding"><span class="no">六</span>方向之外风险在样本内可测：|s_gap|
控制昨日 RV 后预测 SC 日盘 RV（t=4.4，R²=0.64）；但展开窗样本外验证无增量
（§12.5b：ΔR²≈0、CW t≈0）——风险通道降格为样本内证据，其动态杠杆应用
（§12.6）的实用价值随之存疑。</div>
<div class="finding"><span class="no">七</span>可交易性：最优策略（升水回归）净
Sharpe 2.0，但全部策略的 95% CI 含 0——样本长度不足以确立任何 Sharpe；
S5 事件策略经前视勘误重跑后 4bp 成本 Sharpe 约 1.0（CI 含 0，§12.6b）；
固定杠杆不改变 Sharpe 只放大回撤（5x 时 106%），动态杠杆的回撤改善依赖
样本内 σ̂（RV 模型样本外验证未通过，§12.5b）。换手 / 容量 / 成本敏感性
登记见 §12.6b（1 亿元名义约占 SC 日均成交额 0.38%）。</div>
...
（等权变体一致；第三部分已升级时点化累计权重，口径注见 §4.6）。</li>
<li>窗口收益为单点 close/open 标签，未做区间 VWAP 端点对照（导师方法论
优先 VWAP；第三部分分钟级已做双标签，窗口级列为待办）；涨跌停未做截尾
修正，仅有 §3.4 D5 的锁死分钟代理监控。</li>
</ul>
