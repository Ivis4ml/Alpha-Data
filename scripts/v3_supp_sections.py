...
推断（J5 × M 15 分钟格 n = {int(j5['orig_n']):,}，而背后按最窄口径
仅 {int(j5['n_events'])} 次孤立跳、{int(j5['n_days'])} 个交易日；按
审计较宽口径约 53 跳、31 日），前向收益窗口又高度重叠，分钟级 q 值
不能作确认证据。对 J 族唯一显著正时段格（SC 盘中日盘 15 分钟）按
...
J 族全部相关结论从 B 级候选降为<b>探索性证据</b>，正确推断须按
事件 / 日双重聚类重做（登记为待办）。</p>
