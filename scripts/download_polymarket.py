"""下载 HuggingFace 数据集 ``TimeSeventeen/Polymarket-v1`` 的指定层到本地。
...
def _summarize_repo(token: str | None) -> dict[str, tuple[int, int]]:
    """返回仓库顶层目录 -> (文件数, 总字节)。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="下载 Polymarket-v1 指定层")
